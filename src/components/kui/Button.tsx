...
const outerVariants = cva('group inline-flex no-underline', {
  variants: {
    variant: {
      primary:
        'h-10 rounded-[10px] bg-linear-to-b from-[#cfcfcf] to-[silver] transition-colors hover:from-[#e4e4e4] hover:to-[#e4e4e4]',
...
const innerVariants = cva('flex items-center justify-center', {
  variants: {
    variant: {
      primary:
        'm-0.5 rounded-[20px] border border-[#c5c5c5] bg-linear-to-b from-[#b5b5b5] to-[#e4e4e4] transition-colors group-hover:border-[#e4e4e4] group-hover:from-[#e4e4e4] group-hover:to-[#e4e4e4]',
...
const contentVariants = cva(
  "inline-flex items-center font-mono whitespace-nowrap max-[767px]:text-[15px] max-[479px]:text-[14.2px] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        primary:
          'gap-1.5 px-3 pt-[7.5px] pb-[6.5px] text-[13.5px] text-black [-webkit-text-stroke:0.25px_#000] max-[479px]:[-webkit-text-stroke-width:0.2px]',
        secondary:
